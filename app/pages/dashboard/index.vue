...
<template>
  <div class="py-8">
    ...
    <div v-if="isAdmin" class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      ...
      <UCard>
        <div class="flex items-center gap-4">
          <div class="p-3 inline-flex bg-success/10 rounded-xl">
            <UIcon name="i-heroicons-check-circle" class="size-6 text-success" />
          </div>
          <div>
            <p class="text-2xl font-bold text-highlighted">
              Active
      ...
      <UCard>
        <div class="flex items-center gap-4">
          <div class="p-3 inline-flex bg-info/10 rounded-xl">
            <UIcon name="i-heroicons-globe-alt" class="size-6 text-info" />
          </div>
          <div>
            <p class="text-2xl font-bold text-highlighted">
              OIDC
    ...
    <div v-if="!isAdmin && websites?.length" class="mb-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <a
          ...
          target="_blank"
          class="flex items-center gap-3 p-4 bg-elevated rounded-xl border border-default hover:border-primary hover:shadow-md transition-all"
        >
          ...
          <div class="min-w-0 flex-1">
            <p class="font-medium text-highlighted truncate">{{ website.name }}</p>
            <p class="text-sm text-muted truncate">{{ website.websiteUrl }}</p>
          </div>
          <UIcon name="i-heroicons-arrow-top-right-on-square" class="size-4 text-dimmed shrink-0" />
        </a>
      </div>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-6">
      ...
      <UCard>
        <template #header>
          <div class="flex items-center gap-3">
            <div class="p-2 inline-flex bg-muted rounded-lg">
    ...
    <div v-if="isAdmin" class="mt-8 p-4 bg-elevated rounded-xl border border-default">
